...
public interface FriendRequestRepository extends JpaRepository<Friend_requests, Integer> {
	...
	List<Friend_requests> findPendingRequestsByReceiverId(Long userId);
	...
	@Query("SELECT CASE WHEN COUNT(f) > 0 THEN TRUE ELSE FALSE END FROM Friend_requests f " +
	...
	boolean existsFriendship(Long senderId, Long receiverId);
